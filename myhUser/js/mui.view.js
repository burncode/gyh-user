	...
	var View = $.Class.extend({
		...
		_trigger: function(eventType, page) {
			...
			if(this[eventCallbacks].hasOwnProperty(page.id)) {
				...
				for(var len = callbacks.length; len -= 1;) {
		...
		_initNavBar: function() {
			if(this._isAnimateNavbarIOS() && this.navbars) {
				...
				for(var i = 0, len = inners.length; i < len; i += 1) {
					inner = inners[i];
					...
					if(onLeft) {
						center.style.webkitTransform = ('translate3d(' + center.mNavbarLeftOffset + 'px, 0, 0)')
					}
					if(!noLeft) {
						left.mNavbarLeftOffset = -leftWidth;
						left.mNavbarRightOffset = (navbarWidth - leftWidth) / 2;
						if(onLeft) {
							left.style.webkitTransform = ('translate3d(' + left[0].mNavbarLeftOffset + 'px, 0, 0)')
						}
					}
					if(!noRight) {
						...
						if(onLeft) {
							right.style.webkitTransform = ('translate3d(' + right[0].mNavbarLeftOffset + 'px, 0, 0)')
						}
					}
				}
			}
		...
		animateNavbarByIOS: function(percentage) {
			var i, len, style, el;
			for(i = 0, len = this.activeNavElements.length; i < len; i += 1) {
			...
			for(i = 0, len = this.previousNavElements.length; i < len; i += 1) {
